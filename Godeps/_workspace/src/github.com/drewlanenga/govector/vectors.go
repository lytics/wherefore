...
// Push capped will grow a small array to its limited size, or shrink
// a vector's size to the given limit.
func (x *Vector) PushCapped(in float64, limit int) error {
	lenx := len(*x)
	if lenx < limit {
		// Array smaller than limit, use Push to append value
		x.Push(in)
		return nil
	} else if lenx > limit {
		// Vector larger than limit, shrink and PushFixed
		newV := make([]float64, limit, limit)
		sliceX := (*x)[lenx-limit:]
		copy(newV, sliceX)
		*x = newV
		err := x.PushFixed(in)
