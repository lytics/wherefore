...
// If the array is larger than the AnomalyzerConf.VectorCap it
// it will be shrunk, however this should never happen.
func (a *Anomalyzer) PushCapped(x float64) (float64, error) {
	// Add data point to array which will grow to capped size
	err := a.Data.PushCapped(x, a.Conf.VectorCap)
	if err != nil {
		return NA, err
	}
